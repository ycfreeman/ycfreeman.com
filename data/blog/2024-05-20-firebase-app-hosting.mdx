...
But this time, Firebase App Hosting seems to be getting somewhere. Previously I've stumbled upon a problem of hosting the nextjs SSR frontend while the backend is on GKE, and I wanted control on the frontend routing, so I end up just put the nextjs app in a nodejs workload + nginx proxy in front + manually setting up some of those paths in the Ingress... it was just painful.
...
Even tho all compute are free, GCP is charging me for container registry storage... I'm sorry Firebase, it was a good run but I'm back to Vercel for now. Migration took me like 5 minutes and a few clicks in the UI.
